  ...
  const statusInfo = useMemo(() => {
    if (missingSetup) {
      return {
        ...
        tone: 'warning',
        message: setupError || 'Required Ractor credentials are missing. Set RACTOR_HOST_URL and RACTOR_ADMIN_TOKEN before using GitHex.'
      };
    }
...
export async function getServerSideProps(context) {
  ...
  const adminToken = getEnv('RACTOR_ADMIN_TOKEN');
  const ractorHost = getEnv('RACTOR_HOST_URL');
