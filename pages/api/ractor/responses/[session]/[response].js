const REQUIRED_ENV_VARS = ['RACTOR_ADMIN_TOKEN', 'RACTOR_HOST_URL'];
...
function ensureEnv() {
  const missing = REQUIRED_ENV_VARS.filter((key) => !cleanEnv(process.env[key]));
  if (missing.length) {
...
export default async function handler(req, res) {
  ...
  const token = cleanEnv(process.env.RACTOR_ADMIN_TOKEN);
  const host = cleanEnv(process.env.RACTOR_HOST_URL).replace(/\/$/, '');
